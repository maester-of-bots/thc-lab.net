...
import os
from app.github import blueprint
import requests
...
@blueprint.route('/gitpass.html', methods=['GET', 'POST'])
def gitpass():
    return render_template('github/gitpass.html')


@blueprint.route('/gitauth', methods=['GET', 'POST'])
def gitauth():
    args = request.args
    request_token = args.get('code')
    access_token = get_access_token(id, secret, request_token)

    user_data = get_user_data(access_token)
    print(user_data)
    return render_template('github/gitpass.html', userData=user_data)
